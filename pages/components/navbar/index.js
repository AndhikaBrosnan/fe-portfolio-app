import { Box, Flex, IconButton, Image, Spacer, Text } from "@chakra-ui/react";
import { useRouter } from "next/router";
import styles from "./styles.module.css";

const LinkedInLogo = () => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24"
    data-supported-dps="24x24"
    fill="#0a66c2"
    className="mercado-match"
    width="24"
    height="24"
    focusable="false"
  >
    <path d="M20.5 2h-17A1.5 1.5 0 002 3.5v17A1.5 1.5 0 003.5 22h17a1.5 1.5 0 001.5-1.5v-17A1.5 1.5 0 0020.5 2zM8 19H5v-9h3zM6.5 8.25A1.75 1.75 0 118.3 6.5a1.78 1.78 0 01-1.8 1.75zM19 19h-3v-4.74c0-1.42-.6-1.93-1.38-1.93A1.74 1.74 0 0013 14.19a.66.66 0 000 .14V19h-3v-9h2.9v1.3a3.11 3.11 0 012.7-1.4c1.55 0 3.36.86 3.36 3.66z"></path>
  </svg>
);

const Navbar = () => {
  const router = useRouter();

  return (
    <Box pt={1} className={styles["navbar-container"]}>
      <Flex justifyContent="flex-start" alignItems="center" pr={10} pl={10}>
        <Image
          p={1}
          mr={2}
          src="https://dev-secret-app.herokuapp.com/static/media/logo1.93afee01.png"
          alt="logo"
          w="120px"
          h="60px"
        />
        <Text p={1} mr={1} cursor="pointer">
          Home
        </Text>
        <Text p={1} mr={1} cursor="pointer">
          About
        </Text>
        <Text p={1} mr={1} cursor="pointer">
          Experience
        </Text>
        <Text p={1} mr={1} cursor="pointer">
          Education
        </Text>
        <Text p={1} mr={1} cursor="pointerr">
          Certification
        </Text>
        <Spacer />
        <Flex p={1} justifyContent="center" alignItems="center">
          <Text mr={1}>Contact Me</Text>
          <IconButton
            variant="outline"
            colorScheme="linkedin"
            aria-label="Contact Me"
            icon={<LinkedInLogo />}
          />
        </Flex>
      </Flex>
    </Box>
  );
};

export default Navbar;
